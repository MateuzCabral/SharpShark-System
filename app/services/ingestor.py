    ...
    def process(self, filepath: str):
        try:
            if filepath.endswith((".pcap", ".pcapng")):
                ...
                ingestion_executor.submit(_run_ingestion_sync, filepath, self.user_id)
        ...
        try:
            ...
            if folder_to_watch and user_id_to_assign:
                if not os.path.isdir(folder_to_watch):
                    ...
                    event_handler = IngestorEventHandler(user_id=user_id_to_assign)
                    observer = Observer()
...
def _run_ingestion_sync(filepath: str, user_id: str):
    if not os.path.isfile(filepath):
        logger.warn(f"Arquivo {filepath} desapareceu antes do processamento.")
        return
    ...
    try:
        ...
        file_wrapper = UploadFile(
            filename=os.path.basename(filepath),
            file=BytesIO(file_data) 
        )
        
        with SessionLocal() as session:
            _create_file_sync(session, file_wrapper, user_id)
            
            logger.info(f"Arquivo ingerido com sucesso: {filepath}. Criando análise.")
            os.remove(filepath)

    except HTTPException as e:
        if e.status_code == 400 or e.status_code == 413:
            logger.warn(f"Falha ao ingerir {filepath}: {e.detail}. Removendo da pasta.")
            try:
                os.remove(filepath)
            except Exception as rm_e:
                logger.error(f"Não foi possível remover arquivo inválido {filepath}: {rm_e}")
        else:
            logger.error(f"HTTPException ao processar {filepath}: {e.detail}. O arquivo NÃO será removido.")
            
    except Exception as e:
        logger.exception(f"Erro crítico ao processar {filepath}: {e}. O arquivo NÃO será removido. Nova tentativa ocorrerá se o watchdog disparar novamente (ex: modificação).")
        traceback.print_exc()